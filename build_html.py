...
(function() {
  ...
  function checkVersion() {
    fetch('version.json?t=' + Date.now(), { cache: 'no-store' })
      .then(function(r) { return r.json(); })
      .then(function(v) {
        var storedVersion = localStorage.getItem('last_version');
        localStorage.setItem('last_version', v.version);
        // Previous version stored and it changed → reload
        if (storedVersion && v.version !== storedVersion) {
          showToast('🆕 数据已更新，即将刷新...', 'success');
          setTimeout(function() { location.reload(true); }, 1500);
          return;
        }
        // Page data is older than server version → reload
        if (v.updated && DATA.date < v.updated.slice(0, 10)) {
          showToast('🆕 发现新数据(' + v.updated.slice(0,10) + ')...', 'success');
