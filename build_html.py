...
.status-badge { font-size: 32px; font-weight: bold; padding: 8px 20px; border-radius: 8px; }
...
.card h3 { font-size: 16px; color: #8b949e; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px; }
.kpi { text-align: center; }
.kpi .value { font-size: 34px; font-weight: bold; margin: 4px 0; }
.kpi .label { font-size: 13px; color: #8b949e; }
.kpi .sub { font-size: 13px; color: #8b949e; margin-top: 2px; }
...
.risk-tag { display: inline-block; padding: 4px 10px; margin: 4px; border-radius: 4px; font-size: 13px; background: rgba(231,76,60,0.15); color: #e74c3c; border: 1px solid rgba(231,76,60,0.3); }
.advice-box { padding: 12px 16px; border-radius: 8px; font-size: 15px; line-height: 1.6; }
...
.bar-row { display: flex; align-items: center; padding: 4px 0; font-size: 14px; }
...
.freshness { font-size: 12px; color: #8b949e; margin-top: 2px; }
...
var bv = DATA.best_60_val*100;
document.getElementById('kpiBestV').textContent = (bv>=0?'+':'') + bv.toFixed(1) + '%';
...
(function() {
  ...
  // ===== 份额变动表 (消除价格波动影响) =====
  var html2 = '<div style=\"margin-top:12px;margin-bottom:4px;font-size:13px;color:#8b949e;\"> ETF净申购份额 (亿份) — 消除价格波动</div>';
  html2 += '<table style=\"width:100%;font-size:13px;border-collapse:collapse;\">';
  html2 += '<tr style=\"color:#8b949e;font-size:12px;\">';
  html2 += '<th style=\"text-align:left;padding:3px 5px;\">ETF</th>';
  for (var di = 0; di < dates.length; di++) {
    html2 += '<th style=\"text-align:right;padding:3px 6px;\">'+dates[di].slice(5)+'</th>';
  }
  html2 += '<th style=\"text-align:right;padding:3px 6px;color:#58a6ff;\">3日</th>';
  html2 += '<th style=\"text-align:right;padding:3px 6px;color:#58a6ff;\">5日</th>';
  html2 += '<th style=\"text-align:right;padding:3px 6px;color:#58a6ff;\">10日</th>';
  html2 += '</tr>';
  ...
  // Summary
  html2 += '<tr style=\"border-top:2px solid #30363d;font-weight:bold;\">';
  html2 += '<td style=\"padding:3px 5px;\">合计</td>';
  for (var di = 0; di < dates.length; di++) {
    var ds = dates[di], ss = summary[ds+'_s']||0;
    var c = ss > 10 ? '#e74c3c' : (ss < -10 ? '#2ecc71' : '#c9d1d9');
    html2 += '<td style=\"text-align:right;padding:3px 6px;color:'+c+';font-size:13px;\">'+(ss>=0?'+':'')+ss.toFixed(1)+'</td>';
  }
  html2 += '<td style=\"text-align:right;padding:3px 6px;color:'+(summary.d3_s>=0?'#e74c3c':'#2ecc71')+';font-size:13px;\">'+(summary.d3_s>=0?'+':'')+(summary.d3_s||0).toFixed(1)+'</td>';
  html2 += '<td style=\"text-align:right;padding:3px 6px;color:'+(summary.d5_s>=0?'#e74c3c':'#2ecc71')+';font-size:13px;\">'+(summary.d5_s>=0?'+':'')+(summary.d5_s||0).toFixed(1)+'</td>';
  html2 += '<td style=\"text-align:right;padding:3px 6px;color:'+(summary.d10_s>=0?'#e74c3c':'#2ecc71')+';font-size:13px;\">'+(summary.d10_s>=0?'+':'')+(summary.d10_s||0).toFixed(1)+'</td>';
  html2 += '</tr>';
  html2 += '</table>';
  document.getElementById('etfTable').innerHTML += html2;
})();
