...
# Load PE for display
try:
    pe_s = pd.read_csv(PE_CSV, index_col=0, parse_dates=True)['PE-TTM']
    pe_cur = float(pe_s.iloc[-1])
    pe_baseline = pe_s[pe_s.index >= ten_yr_ago]
    pe_pct = (pe_baseline < pe_cur).sum() / len(pe_baseline) * 100 if len(pe_baseline) > 0 else 50
except:
    pe_cur = None; pe_pct = None
...
if pe_cur: print(f"PE: {pe_cur:.2f} | PE分位: P{pe_pct:.1f} (参考)")
...
if missed:
    new_rows = []
    ci = hist['dca_amount'].sum() if len(hist)>0 else 0; cs = hist['shares_bought'].sum() if len(hist)>0 else 0
    for dt, ds in missed:
